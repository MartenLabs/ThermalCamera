...
quantized_model = torch.quantization.quantize_dynamic(
    model,  # 원본 모델
    {torch.nn.Linear},  # 양자화 할 레이어 지정 (선택적)
    dtype=torch.qint8  # 양자화 데이터 유형 지정
)
...
def capture_frame():
    ...
    interpolated_data_array[(interpolated_data_array < 20) | (interpolated_data_array > 40)] = 0
...
def detect_objects(img):
    ...
    results = quantized_model(expanded_img_array)
    # # 'person' 클래스에 해당하는 결과만 저장
    # results.xyxy[0] = [x for x in results.xyxy[0] if int(x[-1]) == 0]
