    ...
    def record_video(self):
        try:
            ...
            while True:
                ...
                # 추가: 일정 간격으로 이미지 파일을 저장하여 나중에 동영상으로 변환
                if frame_count % 30 == 0:
            ...
            # 추가: 이미지 파일들을 동영상으로 변환
            image_files = [os.path.join(output_folder, f'frame_{i:04d}.jpg') for i in range(1, frame_count + 1)]
            if image_files:
                frame = cv2.imread(image_files[0])
                height, width, layers = frame.shape
                video = cv2.VideoWriter(output_video_filename, fourcc, fps, (width, height))
